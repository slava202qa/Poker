    ...
    api.get<any>('/admin/check')
      ...
      .catch((e: any) => {
        ...
        if (msg.includes('403')) {
          ...
          // Retry once — initData may not be ready on first render
          setTimeout(() => {
            api.get<any>('/admin/check')
              .then(() => setAuthState('ok'))
      ...
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="card-surface p-8 text-center max-w-sm">
          ...
          <p className="text-gray-500 text-sm mb-5">
            Нажмите кнопку <b className="text-white">⚙️ Админ панель</b> в боте.
          </p>
          <a href="https://t.me/POKER_VIP_1_Bot" className="btn-gold px-6 py-2.5 text-sm inline-block">
            Открыть бота
          </a>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: '#0a0a0a' }}>
      <div className="px-4 py-3 flex items-center justify-between"
        style={{ background: '#121212', borderBottom: '1px solid rgba(212,168,67,0.15)' }}>
        <div className="flex items-center gap-2">
          <span className="font-extrabold text-poker-gold tracking-wider">ADMIN</span>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button key={tab.path} onClick={() => navigate(tab.path)}
                className="flex-1 py-2 text-center text-[11px] font-bold relative transition-colors rounded-xl"
